...
class DCH(object):
    ...
    def cauchy_cross_entropy(self, u, label_u, v=None, label_v=None, gamma=1, normed=True):
        ...
        if normed:
            ...
            mod_1 = tf.sqrt(tf.matmul(
                reduce_shaper(tf.square(u)), reduce_shaper(tf.square(v)) 
                + tf.constant(0.000001), transpose_b=True))
            ...
            dist = r_u - 2 * tf.matmul(u, tf.transpose(v)) + tf.transpose(r_v) + tf.constant(0.001)
