...
# def set_lr(lr1, lr2, lr3, multiplier, epoch):
# 	if epoch==0:
# 		return lr1*multiplier[0], lr2*multiplier[0], lr3*multiplier[0]
# 	elif epoch<3 and epoch>0:
# 		multiplier = multiplier*(0.94**max(epoch-1,0))
# 		return lr1*multiplier[1], lr2*multiplier[1], lr3*multiplier[1]
# 	else:
# 		multiplier = multiplier*(0.94**max(epoch-3,0))
# 		return lr1*multiplier[2], lr2*multiplier[2], lr3*multiplier[2]
...
for train_stage in train_stages:
	...
	if train_stage == 1:
		...
		hashed_layer = ['hashed_layer.0.weight', 'hashed_layer.0.bias', 'hashed_layer.1.weight', 'hashed_layer.1.bias']
		...
		# sched1 = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[1,4], gamma=0.5)
		# lmbda = lambda epoch: 0.95 ** epoch
		# sched2 = torch.optim.lr_scheduler.StepLR(optimizer, step_size=[1,1], gamma=[0.96,0.96], last_epoch=-1)
	...
	for epoch in range(num_epoch[train_stage-1]):
		# lr_net_1, lr_net_2, lr_cauchy = set_lr(lr_net_1, lr_net_2, lr_cauchy, lr_multipliers, epoch) 
		...
		# sched1.step()
		# sched2.step()
		print("Train mse: {}. MSSSIM: {}. Train hash: {}".format(train_mse,train_msssim,train_hash))
